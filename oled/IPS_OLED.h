...
//-----------------引脚定义------------------------------
//#define IPS_BL_PIN      B7	             //液晶背光引脚定义  ************未接***********
#define IPS_REST_PIN    D14                  //液晶复位引脚定义
#define IPS_DC_PIN      D10	            //液晶命令位引脚定义
...
void ips_writeData(uint8 dat);
void ips_writeIndex(uint8 dat);
...
void ips_showfloat(uint16 x,uint16 y,float num);
...
{
	void (*Init)(void);
	void (*Putchar)(uint16 x,uint16 y,uint8 dat);
	void (*Printf)(uint16 x,uint16 y,uint8 dat[]);
	void (*PrintCharValue)(uint16 x,uint16 y,uint8 dat);
	void (*PrintIntValue)(uint16 x,uint16 y,int16 dat);
	void (*PrintUintValue)(uint16 x,uint16 y,uint16 dat);
	void (*PrintFloatValue)(uint16 x,uint16 y,float num);
	void (*PrintReverseFloatValue)(uint16 x,uint16 y,float num, int8 Bit);
	void (*Showimage)(void);
